<template>
  <section class="form-make-box">
      ...
      <div class="fields-main">
        <div class="fields-box" v-for="item in fields">
          ...
          <draggable
              ...
              item-key="type">
            <div v-for="element in item.fields" :key="element.type" class="field-box" @click="handleWidgetQuickAdd(element)">
    ...
    <!-- 右侧控件设置 -->
    <aside class="form-setting-box">
      <template v-if="showSettingDetail">
        <header class="form-setting-title">{{ (fieldSetting[widget.type] || {}).label || "控件属性" }}</header>
      <div>
      ...
      <div v-else class="form-setting-empty">选择画布中的控件以配置表单属性</div>
    </aside>
  </section>
</template>
...
import CHINA_AREA from "../flow/common/ChinaArea";
import { WIDGET } from "../flow/common/FlowConstant";
import { formulaWidgetVerify } from "../flow/common/FlowFormula";
import Snowflake from "../flow/common/Snowflake";
import Formula from "../flow/dialog/Formula.vue";
...
const handleWidgetQuickAdd = (field) => {
  const addedWidget = handleWidgetClone(field);
  widgets.value.push(addedWidget);
  handleWidgetClick(addedWidget);
};
...
const handleWidgetAdd = (evt) => {
  const addedWidget = Number.isInteger(evt.newIndex) ? widgets.value[evt.newIndex] : undefined;
  if (addedWidget) handleWidgetClick(addedWidget);
};
// 右侧控件设置
...
watch(
    widgets,
    (newVal) => {
      flowDefinition.flowWidgets = newVal;
