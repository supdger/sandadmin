...
| SandAI 管理端 | 使用内置应用目录 `sandadmin-artd/src/views/sand-ai/`；菜单组件路径为 `/sand-ai/...` | Codex | 管理端构建、类型检查和 SandAdmin 宿主页面验收通过 |
| SandAI 发布形态 | 不打包为可安装插件；主应用 API、管理路由与 SandIAM 身份适配均位于 `server/app/` 与 `server/config/` | Codex | 不存在 `sand-ai` 插件元数据或自动加载配置 |
...
SandIAM 仍按插件交付；SandAI 是 SandAdmin 的内置应用。前端路径、后端结构和最终验收必须以这一边界为准。
