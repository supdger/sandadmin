...
'''
    (ROOT / "sandadmin-brand-mark-v3-master.svg").write_text(svg, encoding="utf-8")
...
def write_preview(master: Image.Image) -> None:
    ...
    preview.save(ROOT / "sandadmin-brand-mark-v3-preview.png", optimize=True)


def main() -> None:
    master, anchors = build_master()
    master.save(ROOT / "sandadmin-brand-mark-v3-master.png", optimize=True)
    master.save(ROOT / "sandadmin-brand-mark-v3-1024.png", optimize=True)

    for size in (512, 170):
        image = master.resize((size, size), Image.Resampling.LANCZOS)
        image.save(ROOT / f"sandadmin-brand-mark-v3-{size}.png", optimize=True)
        if size == 170:
            image.save(
                ROOT / "sandadmin-brand-mark-v3-170.webp",
                format="WEBP",
                lossless=True,
                quality=100,
                method=6,
