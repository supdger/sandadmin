...
def width_at(t: float) -> float:
    body = 27 * math.sin(math.pi * t) ** 0.72
    upper_lobe = 9 * math.exp(-((t - 0.28) / 0.16) ** 2)
    lower_lobe = 11 * math.exp(-((t - 0.72) / 0.16) ** 2)
    return 7 + body + upper_lobe + lower_lobe
...
def generate_particles() -> list[str]:
    ...
    for lane, spread, count in ((0.42, 0.18, 500), (-0.38, 0.20, 500)):
...
def main() -> None:
  ...
  <circle cx="355" cy="132" r="15" fill="#71F0E7" opacity="0.38" filter="url(#particleGlow)"/>
  <circle cx="159" cy="399" r="16" fill="#FFD269" opacity="0.42" filter="url(#particleGlow)"/>
