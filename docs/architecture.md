...
`plugins/` 只保存统一仓库清单，不保存业务插件源码或安装包，也不自动加载。本源码工作树不保存业务插件安装副本。插件源码和 Release 位于各自独立权威仓库；安装后的插件目录只存在于消费工作区的演示或隔离宿主。宿主版本由消费方按[宿主发布与消费同步](host-consumer-sync.md)主动拉取并锁定。
