<template>
  <div :class="['form-item-widget-detail-box', widget.required ? 'detail-required' : '']">
    ...
    <draggable
        ...
        v-model="widget.details"
        v-bind="{ group: { name: 'widgets' }, animation: 200, ghostClass: 'ghost' }"
        @add="handleWidgetChange($event)"
        handle=".widget-move"
        item-key="name">
      <div
          v-for="element in widget.details"
          :key="element.name"
          :class="[
            'field-widget',
...
const handleWidgetChange = (evt) => {
  const addedElement = Number.isInteger(evt.newIndex) ? props.widget.details[evt.newIndex] : undefined;
  // 明细不能套明细
  if (props.widget.type === WIDGET.DETAIL) {
    if (addedElement && nonDetailWidgets.includes(addedElement.type)) {
      ArrayUtil.remove(props.widget.details, "name", addedElement.name);
      Notification.warning("暂不支持该控件添加到明细");
      evt = null;
    }
  }
  if (evt && addedElement) emits("click", addedElement);
