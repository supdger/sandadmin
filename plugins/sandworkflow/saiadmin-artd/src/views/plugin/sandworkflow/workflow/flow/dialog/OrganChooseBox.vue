...
<!--            </a-checkbox>-->
            <a-tree
                ...
                :selectable="false"
                :check-strictly="true"
                :field-names="roleTreefieldNames"
                :data="filteredItem(roleTreeData)"
                :show-line="true"
                :checkable="true"
                :multiple="true"
                checked-strategy="all"
...
let props = defineProps({
  ...
  onlyId: { type: Boolean, default: false }, // 选择的值是否是id数组，如果不是则为{id:xx,type:xx}的对象
});
...
const { getDepts, getRoles, getUsers, getAll } = useOrganStore();
...
// 角色树形数据
const roleTreeData = ref([]); // 角色树形数据
const roleCheckedKeys = ref([]); // 选择的角色Key
const roleIds = ref([]); // 选中的角色ids
const roleTreefieldNames = ref({ key: "id", title: "name" }); // 角色树形数据的字段名
...
const initSelected = () => {
  // 全部值
  all.value = store.getAll;;
  // 复制选择的值
  selected0.value = ObjectUtil.copy(props.selected);

  let ids = props.onlyId ? selected0.value : selected0.value.map((i) => i.id);
  // 设置左侧选中
  if (!props.hiddenDept) {
    depts.value = getDepts().map((item) => ({ ...item, type: ORGANS.DEPT.code }));
    deptIds.value = depts.value.map(i => i.id);
  ...
  // if (!props.hiddenRole) {
  //   roles.value = getRoles().map((item) => ({ ...item, type: ORGANS.ROLE.code }));
  //   roles.value.forEach((i) => (i.checked = ids.includes(i.id)));
  // }
  if (!props.hiddenRole) {
    roles.value = getRoles().map((item) => ({ ...item, type: ORGANS.ROLE.code }));
    roleIds.value = roles.value.map((i) => i.id);
    roleTreeData.value = convertToTree(roles.value);
    roles.value.forEach((i) => (i.checked = ids.includes(i.id)));
    roleCheckedKeys.value = roleIds.value.filter((i) => ids.includes(i));
  }
  if (!props.hiddenUser) {
    users.value = getUsers().map((item) => ({ ...item, type: ORGANS.USER.code }));
...
const filteredItem = (items) => {
  if (!keyword.value || !items) {
    ...
    if (type.value === ORGANS.DEPT.code || type.value === ORGANS.ROLE.code) {
...
// };
const onTreeNodeChecked = (checkedKeys, data, type) => {
  const isDept = type === ORGANS.DEPT.code;
  const checkedKeysRef = isDept ? deptCheckedKeys : roleCheckedKeys;
  const idsRef = isDept ? deptIds : roleIds;

  checkedKeysRef.value = checkedKeys;
  let needDeleteIds = idsRef.value.filter((i) => !checkedKeys.includes(i));
  if (props.onlyId) {
    let needAddIds = checkedKeys.filter((i) => !selected0.value.includes(i));
    selected0.value = selected0.value.filter((i) => !needDeleteIds.includes(i));
    needAddIds.forEach((id) => selected0.value.push(id));
  } else {
    let existIds = selected0.value.map((i) => i.id);
    let needAddIds = checkedKeys.filter((i) => !existIds.includes(i));
    selected0.value = selected0.value.filter((i) => !needDeleteIds.includes(i.id));
    needAddIds.forEach((id) => selected0.value.push({ id, type }));
  }
};
...
const convertToTree = (data) => {
  ...
  // deptIds.value = ids;
  if (data[0]?.type === ORGANS.DEPT.code) {
    deptIds.value = ids;
  } else if (data[0]?.type === ORGANS.ROLE.code) {
    roleIds.value = ids;
  }
  return tree;
};
...
const getNameById = (item) => {
  const id = props.onlyId ? item : item.id;
  const type = props.onlyId ? undefined : item.type;
  // const found = all.value.find((i) => i.id === id && (type === undefined || i.type === type));
  const found = store.getById(id, type);
...
const onItemChoosed = (checked, item, type) => {
  ...
  if (props.onlyId) {
    ...
    if (checked) {
      selected0.value.push({ id: item.id, type });
    } else {
      selected0.value = selected0.value.filter((i) => i.id !== item.id || i.type !== type);
    }
  }
};
...
const onItemRemoved = (item) => {
  ...
  if (props.onlyId) {
    ...
    selected0.value = selected0.value.filter((i) => i.id !== id || i.type !== type);
  }
  // deptCheckedKeys.value = deptCheckedKeys.value.filter((i) => i !== id);
  if (type === ORGANS.DEPT.code || type === undefined) {
    deptCheckedKeys.value = deptCheckedKeys.value.filter((i) => i !== id);
    if (!props.hiddenDept) {
      depts.value.forEach((i) => {
        if (i.id === id && i.type === ORGANS.DEPT.code) i.checked = false;
      });
    }
  }
  if (type === ORGANS.ROLE.code || type === undefined) {
    roleCheckedKeys.value = roleCheckedKeys.value.filter((i) => i !== id);
    if (!props.hiddenRole) {
      roles.value.forEach((i) => {
        if (i.id === id && i.type === ORGANS.ROLE.code) i.checked = false;
      });
    }
  }
  // if (!props.hiddenDept) {
  ...
  if (!props.hiddenUser) {
    users.value.forEach((i) => {
      if (i.id === id && i.type === ORGANS.USER.code) i.checked = false;
    });
  }
};
