              ...
              <div class="item-wrap approver">
                <div class="approver-list">
                  <div class="approver-wrapper" v-for="(item, idx) in flowNodeConfig.assignees">
                    ...
                    <div class="main-content">
                      <a-radio-group class="radio-group" v-model="item.assigneeType" @change="onAssigneeTypeChanged(item)">
                        <a-grid :cols="3" :colGap="0" :rowGap="10">
                          ...
                          <a-grid-item
                            ><a-radio :value="1" :disabled="!features.advancedAssignee"
                              >上级</a-radio
                            ></a-grid-item
                          >
                          <a-grid-item
                            ><a-radio :value="2" :disabled="!features.departmentLeader"
                              >部门负责人</a-radio
                            ></a-grid-item
                          >
                          ...
                          <a-grid-item
                            ><a-radio :value="5" :disabled="haveMultiNode || !features.advancedAssignee"
                              >连续多级上级</a-radio
                            ></a-grid-item
                          >
                          <a-grid-item
                            ><a-radio :value="6" :disabled="haveMultiNode || !features.departmentLeader"
                              >连续多级部门负责人</a-radio
                            ></a-grid-item
                          >
                          <a-grid-item
                            ><a-radio :value="7" :disabled="haveMultiNode || !features.initiatorChoice"
                              >发起人自选</a-radio
                            ></a-grid-item
                          >
                        </a-grid>
                    ...
                    <div class="sub-content" v-if="item.assigneeType === 7">
                      <div class="sub-content-top-line"></div>
                      <p class="bold">发起时选择</p>
                      <p class="tip-text">发起审批时由发起人选择 1–25 名处理人；发布后自动生成 choiceKey。</p>
                    </div>
                    <div class="sub-content" v-if="![0, 7].includes(item.assigneeType)">
                      ...
                      <template v-else-if="item.assigneeType === 2">
                        ...
                        <p class="tip-text">按发起人部门解析单人负责人；发布后自动生成 relationKey。</p>
                        <a-form-item v-if="item.relationKey" label="关系键">
                          <a-input :model-value="item.relationKey" disabled />
                        </a-form-item>
                      </template>
                      ...
                      <template v-else-if="item.assigneeType === 4">
                        ...
                        <div class="assignee-box">
                          ...
                          <span class="assignee-list">
                            <a-tag v-for="userId in item.assignees">{{ getUserById(userId.id, userId.type).name }}</a-tag>
                          </span>
                          <OrganChooseBox
                      ...
                      <template v-else-if="item.assigneeType === 6">
                        ...
                        <p class="tip-text">固定依次审批；发布后自动生成 relationKey，发起时无需额外组织参数。</p>
                        <a-form-item v-if="item.relationKey" label="关系键">
                          <a-input :model-value="item.relationKey" disabled />
                        </a-form-item>
                      </template>
                  ...
                  <div class="approver-null-select-box">
                    ...
                    <a-form-item label="审批管理员" v-else-if="flowNodeConfig.flowNodeNoAuditorType === 2">
                      <a-select placeholder="请选择管理员" v-model="flowNodeConfig.flowNodeAuditAdmin">
                        <a-option v-for="userId in workFlowDef.flowAdminIds" :value="userId" :label="getUserById(userId.id, userId.type).name" />
                      </a-select>
                    </a-form-item>
                  </div>
                </div>
          ...
          <div class="content-wrap">
            <div class="item-content">
              <div class="item-wrap auth-list">
                <a-checkbox
                  class="auth-item"
                  v-model="flowNodeConfig.assignable"
                  :value="1"
                  :disabled="!features.assign"
                >
                  允许转交
                  <a-tooltip v-if="!features.assign" content="待转办能力验收后开放">
                    <icon-question-circle class="tip" />
                  </a-tooltip>
                </a-checkbox>
                <a-checkbox
                  class="auth-item"
                  v-model="flowNodeConfig.signable"
                  :value="1"
                  :disabled="!features.addSign"
                >
                  允许加签 / 减签
                  <a-tooltip v-if="!features.addSign" content="待加签/减签能力验收后开放">
                    <icon-question-circle class="tip" />
                  </a-tooltip>
                </a-checkbox>
                <a-checkbox
                  class="auth-item"
                  v-model="flowNodeConfig.backable"
                  :value="1"
                  :disabled="!features.jump"
                >
                  允许回退
                  <a-tooltip v-if="!features.jump" content="待跳转/回退能力验收后开放">
                    <icon-question-circle class="tip" />
                  </a-tooltip>
                </a-checkbox>
                <a-checkbox
                  class="auth-item"
                  v-model="flowNodeConfig.signature"
                  :value="0"
...
import { useFlowStore, useOrganStore } from "../../store";
import { SandWorkflowFeatures } from "../../feature-flags";
...
const features = SandWorkflowFeatures;
...
let onlyOneNode = computed(() => {
  ...
  return [2, 5, 6, 7].includes(fristAssigneeType);
});
...
const onAssigneeTypeChanged = (assignee) => {
  ...
  delete assignee.relationKey;
  delete assignee.choiceKey;
