...
class Depends
{
    ...
    public function getContent(bool $realTime = false): array
    {
        if (!file_exists($this->json)) {
            throw new Exception($this->json . ' 文件不存在!');
        }
        ...
        if (!$this->jsonContent) {
            throw new Exception($this->json . ' 文件读取失败!');
        }
    ...
    public function setContent(array $content = []): void
    {
        ...
        if (!isset($content['name'])) {
            throw new Exception('依赖内容文件内容不完整');
        }
        ...
        if (!$result) {
            throw new Exception('文件没有可写权限:' . $this->json);
        }
    }
        ...
        if (!$cover) {
            foreach ($depends as $key => $item) {
                if (isset($content[$dKey][$key])) {
                    throw new Exception($key . ' 依赖已经存在!');
                }
            }
        }
                ...
                if (is_array($item)) {
                    foreach ($item as $configKey => $configItem) {
                        if (isset($content['config'][$key][$configKey]) && $content['config'][$key][$configKey] != $configItem) {
                            throw new Exception('composer 配置 '.$configKey.' 冲突');
                        }
                    }
                } elseif (isset($content['config'][$key]) && $content['config'][$key] != $item) {
                    throw new Exception('composer 配置 '.$key.' 冲突');
                }
            }
        }
