        ...
        <template #content>
          <div class="add-node-popover-wrap">
            <div class="add-node-popover-item approver" @click="onNodeAddClicked(NODE.APPROVE)">
              <div class="node-icon node-audit">
                <icon-user-add class="iconfont-approval-admin"/>
<!--                <span class="iconfont-approval-admin " v-html="icons[NODE.APPROVE].icon">-->
<!--                </span>-->
              </div>
            ...
            >
              <div class="node-icon node-copy">
                <icon-send class="iconfont-approval-admin"/>
              </div>
              <div class="node-text">抄送人</div>
            </div>
            <div
              v-if="features.transact"
              class="add-node-popover-item transactor"
              @click="onNodeAddClicked(NODE.TRANSACT)"
            >
              <div class="node-icon node-transactor">
                <icon-check-square class="iconfont-approval-admin"/>
              </div>
              <div class="node-text">办理人</div>
            </div>
            <div class="add-node-popover-item condition" @click="onNodeAddClicked(NODE.EXCLUSIVE_GATEWANY)">
              <div class="node-icon node-where">
                <icon-mind-mapping class="iconfont-approval-admin"/>
<!--                <span class="iconfont-approval-admin" v-html="icons[NODE.EXCLUSIVE_GATEWANY].icon"></span>-->
              </div>
  ...
  if (type !== NODE.EXCLUSIVE_GATEWANY) {
    ...
    if (type === NODE.APPROVE) {
      newNode = {
        ...
        assignable: features.assign, // 可转交（未验收默认关闭）
        signable: features.addSign, // 可变签
        backable: features.jump, // 可回退
