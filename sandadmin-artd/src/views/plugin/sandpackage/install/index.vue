    ...
    <ElCard class="flex flex-col flex-1 min-h-0 art-table-card" shadow="never">
      ...
      <div class="flex flex-wrap items-center my-2">
        ...
        <div class="flex items-center gap-1 ml-auto">
          ...
          <div class="version-value">{{ version?.sandadmin_version?.describe }}</div>
          ...
          <div class="version-value">{{ version?.sandpackage_version?.describe }}</div>
          <div class="version-title">状态</div>
          <div
            class="version-value"
            :class="[
...
<script setup lang="ts">
  ...
  const checkVersionCompatibility = (support: string | undefined): boolean => {
    if (!support || !version.value?.sandadmin_version?.describe) {
      return false // 如果没有兼容性信息，默认不允许安装
    }

    const currentVersion = version.value.sandadmin_version.describe
  ...
  const handleInstall = async (record: AppInfo) => {
    ...
    try {
      await sandpackageApi.installApp({ appName: record.app })
      ElMessage.success('安装成功')
      getList()
      sandpackageApi.reloadBackend()
    } catch {
