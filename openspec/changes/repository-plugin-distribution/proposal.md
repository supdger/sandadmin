...
- 新增持久安装存储与实际插件目录合并发现，保留旧根兼容及显式维护迁移；GET不搬迁、不补登记。

- 异常插件提供清理范围预览、显式确认、残留数据库和菜单清理、文件归档及失败续作，完成后允许重新安装。
...
影响 SandPackage 控制器、路由、仓库下载组件、管理界面、打包脚本及仓库治理文档。不增加宿主数据库表；复用既有 PostgreSQL 生命周期。异常清理须管理员确认，发布与隔离验收按本任务有效授权执行，不自动同步消费者或删除现有用户数据。
