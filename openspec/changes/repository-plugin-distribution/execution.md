...
### Permission source repair and pending live application

Authoritative sand_plugins repair is committed/pushed as 21cd4d61c07beee401354e2b20483aa777db7e96 on codex/plugin-cleanup-contracts (initial implementation47ce6366). Migration042 repairs the historical100 permissions and is included in generated install/update payloads, preserving older migration checksums. Independent review required rejecting unknown same-slug row shapes and preserving disabled status; both were fixed, conflicting duplicate statuses rejected. PHP8.2 package integrity26/26 passed. Plugin owner reports isolated rollback-transaction checks for role preservation, idempotence, ambiguity, disabled states and parent ownership; current user database was not migrated.

Root read-only live preflight:100 unique mappings,100 reparentings,95 renames,all target status1;7 destination parents are valid type2/status1 with exact SandIAM components. All215 IAM permission slugs and20543 role links must remain; current instance has no duplicate slugs, so no menu or role deletion is expected. Reviewed application wrapper binds full revision and SQL hash953a647496d2b25d6697e2f6c6aa57c1b2e90370b3c82be19ed0e5bc88543284, locks tables, creates durable checked backup, and verifies IDs/slugs/status, all role rows and non-target menus before commit. Independent backup-path review passed after enforcing JSON errors, full write,fsync and hash read-back. Wrapper has not run; current database migration needs explicit user confirmation per global AGENTS. Preview: task work/cleanup-acceptance-20260921/iam-menu-repair-preview.md.

Workflow/SandAI verified preview assets were mirrored into trusted SandAdmin Releases and independently downloaded with matching hashes. Catalog branch codex/plugin-catalog-preview commit15aaf82 updates Workflow and adds SandAI; actual authenticated3006 refresh shows3 cards and new Workflow tag. Screenshot task work/cleanup-acceptance-20260921/catalog-refreshed.png. IAM market package remains oldv38 pending a new verified immutable package; source repair is not presented as old ZIP replacement.
