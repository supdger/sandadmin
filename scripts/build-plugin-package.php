...
function main(array $argv): void
{
    if (count($argv) < 5 || count($argv) > 6) {
        fail('用法：php scripts/build-plugin-package.php <源码目录> <输出目录> <Release tag> <host_min> [host_max]');
    }
...
$autoload = dirname(__DIR__) . '/server/vendor/autoload.php';
if (!is_file($autoload)) {
    fwrite(STDERR, "server/vendor/autoload.php 不存在，请先安装宿主依赖\n");
    exit(1);
}
require $autoload;
